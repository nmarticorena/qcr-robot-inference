...
class ValidityChecker(ob.StateValidityChecker):
    ...
    def isValid(self, state):
        pose = SE2(state.getX(), state.getY(), state.getYaw())
...
class ReedsSheppPlanner:
    ...
    def __init__(
        ...
        self.space = ob.DubinsStateSpace(turning_radius, False)
    ...
    def plan(
        self, start: SE2, goal: SE2, timeout: float = 1.0, samples: int = 200
    ) -> list[SE2]:
        ...
        self.path = [SE2(s.getX(), s.getY(), s.getYaw()) for s in path.getStates()]
        return self.path
    ...
    def plot(self, ax: plt.Axes | None = None, arrow_every: int = 20) -> plt.Axes:
        ...
        heading = np.array([[np.cos(p.theta()), np.sin(p.theta())] for p in poses[:-1]])
        ...
        for i in range(0, len(poses), arrow_every):
            p = poses[i]
            ax.arrow(p.x, p.y, 0.18 * np.cos(p.theta()), 0.18 * np.sin(p.theta()),
...
if __name__ == "__main__":
    planner = ReedsSheppPlanner(bounds=(-4, 4, -4, 4), turning_radius=0.4, planner= og.RRTstar)
    rrt_connectplanner = ReedsSheppPlanner(bounds=(-4, 4, -4, 4), turning_radius=0.4, planner=og.RRTConnect)
    start = SE2(-2.0, -1.5, np.deg2rad(np.random.uniform(-180, 180)))
    goal = SE2(-1, -1.0, np.deg2rad(np.random.uniform(-180, 180)))
