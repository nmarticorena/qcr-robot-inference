...
class PushTDataset(BaseDataset):
    ...
    def sample_sequence(self, episode: int, buffer_start_idx: int, buffer_end_idx: int) -> dict:
        seq = super().sample_sequence(episode, buffer_start_idx, buffer_end_idx)
        seq["frames"]["frames"] = self.format_images(seq["frames"]["frames"])
        return seq

    @staticmethod
    def format_images(images: NDArray) -> NDArray:
        images = np.asarray(images)
        if images.ndim != 4:
            raise ValueError(f"Expected PushT images with shape T,H,W,C or T,C,H,W, got {images.shape}")
